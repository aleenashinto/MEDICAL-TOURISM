...
export function CareConciergeDrawer({ isOpen, onClose, onOpenIntake }: CareConciergeDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-900/60 backdrop-blur-sm">
      <div className="w-full max-w-md bg-white h-full flex flex-col shadow-2xl border-l border-slate-200 animate-in slide-in-from-right duration-300">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <div className="flex items-center space-x-2">
                  <h3 className="text-sm font-bold text-[#0F2042]">Anjali Menon</h3>
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-100 text-[#0E82FD]">Kerala Concierge</span>
                </div>
                <p className="text-[11px] text-slate-500 font-medium">International Patient Coordinator (Malayalam, English, Arabic, Hindi)</p>
              </div>
            </div>
          ...
          <div className="mt-4 flex items-center justify-between pt-3 border-t border-slate-200">
            <div className="flex rounded-xl bg-slate-200/80 p-1">
              <button 
                ...
                className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${
                  activeTab === 'concierge' ? 'bg-white text-[#0E82FD] shadow-sm' : 'text-slate-600'
                }`}
              >
                Kerala Concierge
              </button>
              <button 
                ...
                className={`px-3 py-1 text-xs font-bold rounded-lg transition-all flex items-center space-x-1 ${
                  activeTab === 'ai' ? 'bg-white text-[#0E82FD] shadow-sm' : 'text-slate-600'
                }`}
              >
            ...
            <div className="flex items-center space-x-2">
              <a 
                ...
                target="_blank" 
                rel="noreferrer"
                className="px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-200 text-xs font-bold flex items-center space-x-1 hover:bg-emerald-100 transition-all"
              >
                <span>WhatsApp</span>
              </a>
              <button className="p-1.5 rounded-lg bg-white border border-slate-200 text-slate-700 hover:text-[#0E82FD]">
